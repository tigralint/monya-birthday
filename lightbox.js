 ...
 * (Код из твоего примера, он отличный и не требует изменений)
 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    imageLinks.forEach(link => {
        link.addEventListener('click', e => {
            ...
            // 4. Показываем подпись (из `alt` картинки-превью внутри ссылки)
            const previewImage = link.querySelector('img');
            let captionText = '';
            if (previewImage) {
                // Ищем 'alt' у превью-картинки
                captionText = previewImage.getAttribute('alt');
            }
            if (captionText) {
